                   ...
                   uint64_t rand_mask)
{
  ...
  for (int i=0; i<KYBER_SYMBYTES/8; i++) {
    ((uint64_t*)buf)[i] = victim_pointer[i] ^ rand_mask;
  }

  /* coins are in kr+KYBER_SYMBYTES */
  indcpa_enc_attack(ct, buf, pk, kr+KYBER_SYMBYTES, index, flip);
                  ...
                  uint64_t rand_mask)
{
  ...
  /* coins are in kr+KYBER_SYMBYTES */
  indcpa_enc_fake(ct, buf, pk, kr+KYBER_SYMBYTES, index, guess, poly_idx);

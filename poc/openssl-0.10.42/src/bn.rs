...
foreign_type_and_impl_send_sync! {
    type CType = ffi::BN_MONT_CTX;
    fn drop = ffi::BN_MONT_CTX_free;

    /// [`BN_MONT_CTX`]: https://www.openssl.org/docs/man1.1.0/crypto/BN_MONT_CTX_new.html
    pub struct BigNumMontgomeryContext;

    /// [`BigNumMontgomeryContext`]: struct.BigNumMontgomeryContext.html
    pub struct BigNumMontgomeryContextRef;
}

impl BigNumMontgomeryContext {
    /// Returns a new `BigNumMontgomeryContext`
    #[corresponds(BN_MONT_CTX_new)]
    pub fn new() -> Result<BigNumMontgomeryContext, ErrorStack> {
        unsafe {
            ffi::init();
            cvt_p(ffi::BN_MONT_CTX_new()).map(BigNumMontgomeryContext)
        }
    }
}

impl BigNumMontgomeryContextRef {
    /// Initialize a new `BigNumMontgomeryContext`
    #[corresponds(BN_MONT_CTX_set)]
    pub fn init(&mut self,
        modulus: &BigNumRef,
        ctx: &mut BigNumContextRef
    ) -> Result<(), ErrorStack> {
        unsafe {
            cvt(ffi::BN_MONT_CTX_set(
                self.as_ptr(),
                modulus.as_ptr(),
                ctx.as_ptr()
            )).map(|_| ())
...
impl BigNumRef {
    ...
    #[corresponds(BN_from_montgomery)]
    pub fn from_montgomery(
        &mut self,
        a: &BigNumRef,
        mont_ctx: &mut BigNumMontgomeryContextRef,
        ctx: &mut BigNumContextRef) -> Result<(), ErrorStack> {
        unsafe { cvt(ffi::BN_from_montgomery(
            self.as_ptr(),
            a.as_ptr(),
            mont_ctx.as_ptr(),
            ctx.as_ptr()))
    ...
    #[corresponds(BN_to_montgomery)]
    pub fn to_montgomery(
        &mut self,
        a: &BigNumRef,
        mont_ctx: &mut BigNumMontgomeryContextRef,
        ctx: &mut BigNumContextRef) -> Result<(), ErrorStack> {
        unsafe { cvt(ffi::BN_to_montgomery(
            self.as_ptr(),
            a.as_ptr(),
            mont_ctx.as_ptr(),
            ctx.as_ptr()))
    ...
    #[corresponds(BN_mod_exp_mont_word)]
    pub fn mul_mont(
        &mut self,
        a: &BigNumRef,
        b: &BigNumRef,
        mont: &mut BigNumMontgomeryContextRef,
        ctx: &mut BigNumContextRef,
    ) -> Result<(), ErrorStack> {
        unsafe {
            cvt(ffi::BN_mod_exp_mont_word(
                self.as_ptr(),
                0,
                a.as_ptr(),
                b.as_ptr(),
    ...
    #[corresponds(BN_mod_exp_mont_consttime)]
    pub fn mod_exp_mont_consttime(
        &mut self,
        a: &BigNumRef,
        p: &BigNumRef,
        m: &BigNumRef,
        ...
        unsafe {
            cvt(ffi::BN_mod_exp_mont_consttime(
                self.as_ptr(),
                a.as_ptr(),
                p.as_ptr(),
                m.as_ptr(),
...
impl BigNum {
    ...
    pub fn from_dec_str(s: *mut u64) -> Result<BigNum, ErrorStack> {
        unsafe {
            ffi::init();
            // let c_str = CString::new(s.as_bytes()).unwrap();
            let mut bn = ptr::null_mut();
            cvt(ffi::BN_dec2bn(&mut bn, s as *const _))?;
